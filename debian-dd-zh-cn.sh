...
# DNS 选择（用户选择：默认为国内镜像源）
ALIYUN_NS_V4="223.5.5.5 223.6.6.6"
TENCENT_NS_V4="119.29.29.29 182.254.116.116"
GOOGLE_NS_V4="8.8.8.8 1.1.1.1"
ALIYUN_NS_V6="2400:3200::1 2400:3200:baba::1"
TENCENT_NS_V6="2402:4e00:: 2402:4e00:1::"
GOOGLE_NS_V6="2001:4860:4860::8888 2606:4700:4700::1111"

echo -ne "\n${aoiBlue}DNS 配置${plain}\n"
echo "请选择 DNS 服务器："
echo "1) 阿里云 DNS (推荐国内用户)"
echo "2) 腾讯云 DNS (推荐国内用户)"
echo "3) Google DNS (推荐海外用户)"
echo "4) 使用当前系统 DNS"
read -p "请输入选择 [默认 1]: " dns_choice
...
case "$dns_choice" in
    ...
    4)
        ...
        # 如果任一协议族缺失，用阿里云默认值补充
        [ -z "$NS_V4" ] && NS_V4="$ALIYUN_NS_V4"
        [ -z "$NS_V6" ] && NS_V6="$ALIYUN_NS_V6"
        ;;
    *)
        NS_V4="$ALIYUN_NS_V4"
        NS_V6="$ALIYUN_NS_V6"
...
if [ -z "$NAMESERVERS" ]; then
    NAMESERVERS="$ALIYUN_NS_V4 $ALIYUN_NS_V6"
fi
# 准备 systemd-networkd DNS 行（为 resolved 的每链路 DNS）
NETWORKD_DNS_LINE="DNS=$NAMESERVERS"

echo -en "\n${aoiBlue}选择下载源${plain}\n"
echo "请选择下载源（用于下载启动文件）："
echo "1) 阿里云镜像 (推荐国内用户)"
echo "2) 清华大学镜像 (推荐国内用户)"
echo "3) 网易镜像 (推荐国内用户)"
echo "4) Debian 官方源 (推荐海外用户)"
read -p "请输入选择 [默认 1]: " mirror_choice
...
case "$mirror_choice" in
    ...
    2)
        ...
        ;;
    3)
        MIRROR_BASE="https://mirrors.163.com/debian"
        ;;
    4)
        MIRROR_BASE="https://ftp.debian.org/debian"
        ;;
    *)
        MIRROR_BASE="https://mirrors.aliyun.com/debian"
        ;;
...
# 尝试从选择的镜像源下载文件
LINUX_URL="$MIRROR_BASE/dists/$debian_version/main/installer-amd64/current/images/netboot/debian-installer/amd64/linux"
INITRD_URL="$MIRROR_BASE/dists/$debian_version/main/installer-amd64/current/images/netboot/debian-installer/amd64/initrd.gz"

# 尝试下载，如果失败则尝试其他镜像源
download_success=false

for try_mirror in "$MIRROR_BASE" "https://mirrors.aliyun.com/debian" "https://mirrors.tuna.tsinghua.edu.cn/debian" "https://ftp.debian.org/debian"; do
    echo "尝试从 $try_mirror 下载..."
    LINUX_URL="$try_mirror/dists/$debian_version/main/installer-amd64/current/images/netboot/debian-installer/amd64/linux"
    INITRD_URL="$try_mirror/dists/$debian_version/main/installer-amd64/current/images/netboot/debian-installer/amd64/initrd.gz"
    
    if wget -q -O linux "$LINUX_URL" && wget -q -O initrd.gz "$INITRD_URL"; then
        echo "下载成功！"
        download_success=true
        break
    else
        echo "从 $try_mirror 下载失败，尝试下一个镜像源..."
        rm -f linux initrd.gz  # 清理失败的下载
    fi
done

if [ "$download_success" = false ]; then
    echo "错误：所有镜像源均下载失败，请检查网络连接。" >&2
    exit 1
fi
...
case "$mirror_choice" in
    1)
        INSTALL_MIRROR="mirrors.aliyun.com"
        ;;
    2)
        INSTALL_MIRROR="mirrors.tuna.tsinghua.edu.cn"
        ;;
    3)
        INSTALL_MIRROR="mirrors.163.com"
        ;;
    4)
        INSTALL_MIRROR="deb.debian.org"
        ;;
    *)
        INSTALL_MIRROR="mirrors.aliyun.com"
        ;;
...
d-i mirror/http/hostname string $INSTALL_MIRROR
